    ...
    <section>
      ...
      <div class="space-y-3">
        ...
        <div>
          ...
          <!-- Notched track: tap a notch or drag the thumb to pick effort. -->
          <div
            ref="effortTrack"
            role="slider"
            tabindex="0"
...
const effortMeta: Record<EffortLevel, { short: string; label: string; hint: string }> = {
  low: {
    short: 'Low',
    label: 'Low',
    hint: 'One launch per slot per day — strongly favors long, low-maintenance sends.',
  },
  medium: {
    short: 'Med',
    label: 'Medium',
  ...
  high: {
    short: 'High',
    label: 'High',
    hint: 'One launch per slot per hour — assumes you relaunch promptly.',
  },
  max: {
    short: 'Max',
    label: 'Max',
    hint: 'As often as the optimizer wants — assumes you relaunch the instant a mission lands.',
  },
};
