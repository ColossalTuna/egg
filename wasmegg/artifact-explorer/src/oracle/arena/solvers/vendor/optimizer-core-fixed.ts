...
import type { LaunchOption, LaunchSolution, OptimizerSolution, RecipeDAG, SlotSummary } from '../../../../lib/types';
import { ei } from 'lib';
import {
  alphaToProb,
  compileJointInnerLp,
...
// Scored-trial ceiling, counted in evalScoreAt calls. This was a 15ms wall-clock
// deadline, which made the whole solver machine- and load-dependent: identical
// inputs returned different plans depending on what else the box was doing, so
// no invariant over the solver could be asserted without flaking. Successor
// generation is the only unbounded work here (pack tests are already capped by
// POLISH_MAX_PACK_TRIALS x POLISH_BEAM_WIDTH), so metering it is enough to make
// polish deterministic. Set well above the structural maximum a normal instance
// reaches, so it binds only on pathological ones; see optimizer-perf.spec.ts for
// the latency this buys.
const POLISH_MAX_EVALS = 20_000;
...
// Deliberately far below packWitness's default: polish runs inside a bounded
// trial budget and treats an undecided instance as a rejection, so a cheap
// "don't know" is worth more here than a slow exact answer.
const POLISH_PACK_NODE_BUDGET = 20_000;

// Contention band on top of the flat top-POLISH_BEAM_WIDTH cut: a successor
// scoring within this relative probability gap of the level's best stays in
// the beam past rank 4, because the best-scoring immediate move is not always
// the one on the path to the best eventual plan (see the exchange comment on
// polish() below). This is what B1-option-order and A3-menu turned out to be:
// two runs generating the identical successor set could still promote
// different nodes because a flat rank-4 cut discards a close-but-not-quite-
// top-4 successor arbitrarily. Bounded by POLISH_MAX_PACK_TRIALS rather than
// a new count of its own, so an uncontested level -- the common case --
// costs exactly what it always did; only a genuinely close field pays more.
const POLISH_BEAM_REL_SLOP = 0.02;

// Reserved capacity in buildPolishCandidates for options adjacent to the
// incumbent's own support (same ship, another duration; or same target,
// another ship). topAlone and dualPruned both rank candidates by *standalone*
// score, which is structurally blind to an option whose value is
// combinatorial rather than solo -- it can rank last in every source list
// there and never surface, which is exactly what starved A3-menu's pivotal
// option. invariants.ts's D2 check uses this identical adjacency rule for the
// identical reason. Additive, not carved out of POLISH_MAX_CANDIDATES: that
// budget is already the thing crowding candidates out, so a second cap
// fighting the first one over the same slots would just relocate the
// crowding instead of fixing it.
//
// Sized to match POLISH_MAX_CANDIDATES rather than picked independently: the
// adjacency pool is ranked by how *many* of the two adjacency relations an
// option satisfies, with the option id as the tie-break, and that tie-break
// carries no information about value. So within an adjacency tier the cut is
// arbitrary, and a wider menu inserts more same-tier options ahead of the one
// that mattered. On invariant:2000 that is exactly what the residual A3-menu
// violation was: hiding ATREGGIES shrinks the tier-1 pool from 27 to 18 and
// moves galeggtica-short::11 from rank 16 to rank 7, i.e. from outside a
// width-8 cut to the last slot inside it. A cap of 8 was fitted to the pool
// of a *narrow* menu; giving this source the same budget as the ranked
// sources above is the smallest rule that is stated rather than fitted.
// Note this raises the threshold, it does not remove the property: any hard
// cap over a value-blind intra-tier order stays violable in principle.
const POLISH_ADJACENCY_CANDIDATES = 20;
...
interface CoreResult {
  ...
  // Dominance survivors (allSurvivors), for callers that need to know which
  // options were ever live at all -- a dominance-pruned option can never be
  // uniquely necessary (whatever dominates it is a weak improvement on every
  // dimension), so ranking it alongside real candidates would be meaningless.
  survivors: ReadonlySet<number>;
  // LP sensitivity value per option: 0 for LP-support members (the maximum,
  // by complementary slackness) and the reduced cost otherwise. Exposed so a
  // caller can rank options by LP-informed value instead of array position or
  // option identity -- see buildPolishCandidates' adjacency tier.
  reducedCost: Float64Array;
}
...
// Total order on an option's own identity (`id` is `missionTypeId::target`,
// stable and content-derived -- see phases.ts's makeLaunchOption). Used only
// as a tie-break beneath a primary sort key, so that an exact tie resolves
// the same way regardless of where the option happens to sit in the array.
// B1-option-order exists precisely because several ranking sorts in this file
// used to fall through to array position on a tie, which made their output --
// and everything sliced off it -- a function of enumeration order rather than
// of the options themselves.
function compareOptionIdentity(a: LaunchOption, b: LaunchOption): number {
...
export function optimizeFull(args: OptimizeArgs): OptimizerSolution {
  ...
  if (feasibleOptions.length > 0 && S > 0) {
    ...
    const polishCandidates = POLISH_ENABLED ? buildPolishCandidates([relaxed, floor], ctx.options, bestAlloc) : [];
    if (POLISH_ENABLED) {
...
function coreSearch(ctx: EvalContext, R: number, S: number, epsilon: number): CoreResult {
  ...
  const byAlone = (x: number, y: number) => {
    ...
    return compareOptionIdentity(options[x], options[y]);
  };
  ...
  // A single LP solve returns exactly one optimal vertex. When the relaxation
  // is near-degenerate -- several vertices within a hair of each other in
  // relaxed value -- the vertex the simplex happens to terminate on can round
  // and repair to a materially worse integer plan than one it never visits.
  // Confirmed on A3-menu, invariant:2014: adding one ship flipped the vertex
  // onto a basis only 0.066% better in the relaxation (well inside the
  // tangent envelope's own ~2.5% documented slack, so that preference is not
  // even reliably meaningful) but 5.8% worse after floor-rounding and repair
  // -- and the alternate vertex was never tried at all, by this stage or any
  // other: it was not crowded out of any capped list, it simply never had a
  // chance to be a rounding seed. Force one alternate: drop the current
  // vertex's largest support member and re-solve over the rest, which is
  // what surfaces a genuine substitute for that budget share -- a different
  // option or combination -- rather than the same vertex minus one unit.
  // Dropping it from the LP does not block repairAlloc from adding it back;
  // this only forces the relaxation to try living without it once.
  //
  // Gated the same way the triple scan is: skip when the search already
  // believes it is within epsilon of the LP bound, since a strictly weaker
  // relaxation (one fewer column) is not going to beat an already-converged
  // incumbent, and the extra solve is not worth paying for on those instances.
  if (lp.certified && lpSupport.size > 0 && relativeProbGap(lp.F, bestScore) > epsilon) {
    // Ties broken by option identity, not array position -- the same
    // anti-pattern B1-option-order exists to catch elsewhere in this file.
    // Not observed to fire in campaign testing (a tie in lp.x at a degenerate
    // vertex is a narrower condition than the ranking ties byAlone guards
    // against), but there is no reason to leave the pattern in code written
    // in the same pass that purged it everywhere else.
    let heaviest = -1;
    let heaviestX = -Infinity;
    for (let s = 0; s < allSurvivors.length; s++) {
      const i = allSurvivors[s];
      if (!lpSupport.has(i)) continue;
      const x = lp.x[s];
      if (x > heaviestX || (heaviest >= 0 && x === heaviestX && compareOptionIdentity(options[i], options[heaviest]) < 0)) {
        heaviestX = x;
        heaviest = i;
      }
    }
    if (heaviest >= 0) {
      const altSurvivors = allSurvivors.filter(i => i !== heaviest);
      const altLp = solveRelaxationLp(altSurvivors, options, innerLp, R, S, baseYield, targets, recipeDag, QByTarget);
      if (altLp.certified) {
        const altRounded = new Map<number, number>();
        for (let s = 0; s < altSurvivors.length; s++) {
          const k = Math.floor(altLp.x[s]);
          if (k > 0) altRounded.set(altSurvivors[s], k);
        }
        let altScore = evalScoreAt([...altRounded]);
        altScore = repairAlloc(altRounded, altScore, options, R, S, evalScoreAt);
        if (altScore > bestScore + ZERO_TOL) {
          bestScore = altScore;
          bestAlloc = altRounded;
          // `support` deliberately stays the original vertex's, not altLp's:
          // altLp.support is computed over altSurvivors, which structurally
          // excludes `heaviest` by construction, so reassigning here would
          // permanently drop it from buildPolishCandidates' and
          // escalatePacking's priority pools even when repairAlloc has just
          // put it back into bestAlloc -- confirmed as a real regression
          // (two new A1-fuel violations) rather than the harmless
          // consistency cleanup it looked like.
        }
      }
    }
  }
  ...
  return {
    bestAlloc,
    bestScore,
    U: lp.certified ? lp.F : Infinity,
    support: lpSupport,
    dualPruned,
    topAlone,
    survivors: new Set(allSurvivors),
    reducedCost,
  };
}
...
// against this cap.
//
// A fourth source, adjacency to `incumbent`, is layered on top in its own
// reserved capacity (POLISH_ADJACENCY_CANDIDATES) rather than competing for a
// slot in the capped list above: support/topAlone/dualPruned all rank by a
// score that is either the option's own solo value or an LP sensitivity bound
// on adding one unit of it alone, so an option whose real value only shows up
// in combination with something already held can rank at or near the very
// bottom of every one of those lists — see A3-menu, where the pivotal option
// ranked last (12th of 12) in its best-placed source list and so never made
// the cut on a wide menu even though it was needed on a narrower one.
function buildPolishCandidates(
  parts: CoreResult[],
  options: LaunchOption[],
  incumbent: Map<number, number>
): number[] {
  const out: number[] = [];
  const seen = new Set<number>();
  const push = (i: number) => {
  ...
  const excluded = new Set(out);
  const held: number[] = [];
  for (const [i, k] of incumbent) {
    if (k <= 0) continue;
    held.push(i);
    excluded.add(i); // polish() always considers held options itself; no need to add them here too
  }
  if (held.length > 0) {
    // A dominance-pruned option can never be uniquely necessary -- whatever
    // dominates it costs no more and yields at least as much on every
    // dimension -- so it is excluded here rather than ranked alongside real
    // candidates, where its LP sensitivity value defaults to 0 for the wrong
    // reason (never computed, not because it is at the LP optimum) and would
    // be indistinguishable from a genuine support member.
    const liveSurvivors = new Set<number>();
    for (const p of parts) for (const i of p.survivors) liveSurvivors.add(i);
    ...
    // LP sensitivity value, best of both calls' relaxations: 0 for a support
    // member (the maximum, by complementary slackness), the reduced cost
    // otherwise. This is what makes the tier value-aware instead of
    // alphabetical -- a fixed-width cap over an order that carries no value
    // information is violable at any width (see A3-menu on invariant:2000,
    // ship ATREGGIES: the pivotal option ranked mid-table by this value but
    // outside a width-8 alphabetical cut once nine higher-alphabetical,
    // unrelated options entered the pool).
    const meritOf = (i: number): number => {
      let best = -Infinity;
      for (const p of parts) {
        const v = p.support.has(i) ? 0 : p.reducedCost[i];
        if (v > best) best = v;
      }
      return best;
    };
    const adjacent = options
      .map((_, i) => i)
      .filter(i => liveSurvivors.has(i) && !excluded.has(i) && adjacency(i) > 0)
      .sort(
        (a, b) =>
          adjacency(b) - adjacency(a) || meritOf(b) - meritOf(a) || compareOptionIdentity(options[a], options[b])
      )
      .slice(0, POLISH_ADJACENCY_CANDIDATES);
    for (const i of adjacent) out.push(i);
  }
...
// Content-keyed counterpart to allocSignature, used only to break a sort tie
// between two allocations with the same score. allocSignature itself stays
// index-keyed: `visited` only needs a stable identity within one run, never a
// comparison across runs or option orderings, and re-deriving option ids for
// every visited-set insertion would be pure overhead there.
function allocIdentitySignature(alloc: Map<number, number>, options: LaunchOption[]): string {
  const pairs: [string, number][] = [];
  for (const [i, k] of alloc) if (k > 0) pairs.push([options[i].id, k]);
  pairs.sort((a, b) => (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0));
  let sig = '';
  for (const [id, k] of pairs) sig += id + ':' + k + ',';
  return sig;
}
...
function polish(start: PackResult, ctx: SearchContext, R: number, S: number, candidates: number[]): PackResult {
  ...
  for (let depth = 0; depth < POLISH_MAX_DEPTH && beam.length > 0; depth++) {
    if (evals >= POLISH_MAX_EVALS) break;
    ...
    for (const node of beam) {
      ...
      const consider = (drop: number, add: number) => {
        ...
        // Budget checked before the visited mutation, so an unscored successor
        // is not permanently retired from the search.
        if (evals >= POLISH_MAX_EVALS) return;
        visited.add(sig);
        evals++;
    ...
    // A tie here must not resolve by generation order, which traces back to
    // addList order and therefore to option array order — the same defect
    // byAlone above has, and the direct cause of B1-option-order surviving
    // even after buildPolishCandidates' own tie-breaks were fixed.
    successors.sort((a, b) => {
      if (b.score !== a.score) return b.score - a.score;
      return allocIdentitySignature(a.alloc, options) < allocIdentitySignature(b.alloc, options) ? -1 : 1;
    });

    const next: PolishNode[] = [];
    let packTests = 0;
    // Keep the top POLISH_BEAM_WIDTH by score, as always, but do not let a
    // flat rank cut discard a successor that is genuinely close behind: the
    // whole reason this stage is a beam and not a hill climb is that the
    // best-scoring immediate move is not always the one on the path to the
    // best eventual plan (see the comment on polish() above). A successor
    // within POLISH_BEAM_REL_SLOP of this level's best stays live regardless
    // of rank. POLISH_MAX_PACK_TRIALS remains the one hard cap — on an
    // uncontested level (the common case, where nothing is close behind the
    // top POLISH_BEAM_WIDTH) this promotes exactly the same nodes it always
    // did; only a genuinely close field spends the extra pack tests.
    const topScore = successors[0].score;
    for (const s of successors) {
      if (packTests >= POLISH_MAX_PACK_TRIALS) break;
      const inCore = next.length < POLISH_BEAM_WIDTH;
      const inSlopBand = relativeProbGap(topScore, s.score) <= POLISH_BEAM_REL_SLOP;
      if (!inCore && !inSlopBand) break; // successors is sorted descending: nothing further can qualify either
      packTests++;
