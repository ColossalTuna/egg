...
import { candidateTargets, mulberry32 } from '../generate';
...
export function generateInstance(seed: number): ArenaInstance {
  ...
  config.epicResearchFTLLevel = randInt(0, 60);
  config.epicResearchZerogLevel = randInt(0, 10);
  ...
  for (const s of spaceshipList) {
    config.shipLevels[s] = randInt(0, shipMaxLevel(s));
  ...
  return {
    ...
    targets,
    config,
    craftingLevel: randInt(1, 30),
    previousCrafts: pick([0, 10, 50, 100, 300]),
    fuelCapacity: fuelTankSizes[randInt(2, fuelTankSizes.length - 1)],
    timeCapacity: randInt(1, 30) * 86400,
