...
describe('optimizeFull', () => {
  ...
  it('reaches the brute-force optimum on a tight-fuel mix', () => {
    ...
    // Fuel binds at 6; time (three slots of 8s) is ample. The brute-force
    // optimum is 6x opt1 -> min(B,C) = 12.18 crafts; opt0/opt2 are correctly
    // dropped. The solver must reach that optimum (a dual filter that wrongly
    // pruned opt1 would fall short).
