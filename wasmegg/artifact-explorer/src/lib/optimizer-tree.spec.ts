// DAGs and solutions are hand-built over the real lunar totem tier chain,
// since the builders resolve names and icons through getArtifactTierPropsFromId.
...
// lt4 needs 2x lt3 + 1x lt2; both intermediates share the lt1 leaf, tied at
// depth 2 from lt4 via either parent — lt3 is enqueued (and so dequeued)
// first, so it wins the canonical occurrence.
...
describe('computeCanonicalOccurrence', () => {
  ...
  it('breaks depth ties deterministically by BFS dequeue order (first parent wins)', () => {
    // lt1 is reachable at depth 2 via both lt3 and lt2; lt3 is lt4's first
    // child, so it is enqueued and dequeued first.
...
describe('buildRecipeTree', () => {
  ...
  it('expands the canonical occurrence of a shared leaf and dims the duplicate, without recursing into it', () => {
    ...
    // same nodeId, same metrics — but this is the deeper/duplicate occurrence
  ...
  it('tolerates cycles between ingredients without infinite recursion', () => {
    ...
    // lt3's canonical child is lt2, but lt2 is already canonically expanded
    // as lt4's own child, so lt3's occurrence of lt2 is the duplicate one.
          ...
          children: [
            {
              ...
              qtyPerParentCraft: 2,
              isLeaf: true,
              // shared leaf's shallowest occurrence is via lt3 above; this
              // one renders inline (dimmed) with no re-expanded children
              isDuplicate: true,
              metrics: { have: 5 },
              children: [],
            },
...
describe('computeCraftChainTree', () => {
  ...
  it('breaks down owned/dropped/crafted/consumed per node, root included', () => {
    ...
    expect(lt1ViaLt2.isDuplicate).toBe(true);
    expect(lt1ViaLt2.children).toEqual([]);
    // same metrics computation, just rendered inline instead of expanded
    expect(lt1ViaLt2.metrics).toEqual(lt1ViaLt3.metrics);
  });
