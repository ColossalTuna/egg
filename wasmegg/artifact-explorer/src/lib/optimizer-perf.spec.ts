...
// Production-scale latency guard for the outer solver. The tight 100ms bar is
// gated behind RUN_PERF=1 (CI hardware and shared runners are noisy); an
// always-on loose cap catches gross regressions without flaking.
const STRICT = process.env.RUN_PERF === '1';
const LOOSE_CAP_MS = 300;
const STRICT_CAP_MS = 100;

// A deep, many-ingredient target with every ship visible (perfectShipsConfig)
// and a long horizon maximizes the option count and the per-option launch
// ranges — the worst case for the search + packing pipeline.
